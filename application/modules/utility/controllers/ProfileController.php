		...
		if($this->getRequest()->isPost()){
			...
			if($rs['code']=="SUCCESS"){
				...
				if(!empty($studentRS)){
					...
					Application_Form_FrmMessage::redirectUrl("/home?message=SUCCESS_ADDED_NEW_ACCOUNT");	
				}
			}
		...
		if(!empty($arrayStudentList)){
			...
			foreach($arrayStudentList AS $accRow){
				if($stuId==$accRow['id']){
					...
					Application_Form_FrmMessage::redirectUrl("/home?message=SWTICHING_STUDENT_SUCCESSFULL");	
					exit();
				}
			}
		}
